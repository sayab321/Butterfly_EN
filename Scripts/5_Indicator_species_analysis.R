...
# data
ind_data <- read_excel("~/Desktop/Pratiksha/Data/5_Ind_data.xlsx")

# prepare data
# Columns 3 to 14 are May(C,F,S), June(C,F,S), July(C,F,S), August(C,F,S)
species_names <- ind_data$Species
data_for_isa <- as.data.frame(t(ind_data[, 3:14]))
...
# Save Final Output
write.csv(top_indicators, "~/Desktop/Pratiksha/Final_Indicator_Analysis_Detailed.csv", row.names = FALSE)
